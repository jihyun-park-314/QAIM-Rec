...
> **v0.4.23 (v0.4.22 self-reconstruction FORBIDDEN 정정 — 대조+frozen에서 void·데이터 확증 · 진짜 게이트=code blocker · main은 β 경험비교로 이연)** — v0.4.22가 `X_in_seq` 87.7%를 self-reconstruction으로 보고 FULL TRAINING을 FORBIDDEN한 근거를 정정한다. self-reconstruction은 대조손실+frozen backbone에서 *기계적으로 불가능*하며 STEP2G 데이터가 이를 확증한다. FORBIDDEN의 진짜 근거는 `X_in_seq`가 아니라 미해결 code-audit blocker다. **이 블록이 본문·v0.4.22·이전 changelog의 상충 기술을 우선한다.**
>
> - **★self-reconstruction FORBIDDEN 정정 (3겹 근거)**: v0.4.22 표의 "X_in_seq=True 87.7% → self-reconstruction → FORBIDDEN"은 틀렸다. (a) **대조 상쇄** — `L_mem`은 `m+`/`m-` 대조이고 `seq`가 둘 사이에 고정 → `X`가 `seq`에 있어 `z`가 부풀어도 그 baseline이 `z+`/`z-`에 *동일하게* 들어가 `Δz=z+−z-`에서 상쇄된다. 손실이 보는 건 메모리 차이뿐. (b) **frozen backbone** — Stage2는 SASRec frozen(plan §M3, v0.4.16 코드입증). self-reconstruction(=seq로 X를 외우기)은 seq 경로 gradient가 필요한데 `item_emb`·attention·`seq` 임베딩이 전부 frozen이라 *그 경로의 학습 파라미터가 0개* → 불가능. `L_mem`이 만지는 건 prefix(projector/encoder)뿐. (c) **데이터 확증** — self-recon이 신호를 부풀렸다면 in-seq `pos_frac`이 out-seq보다 *훨씬 높아야* 하나, 실측 **in-seq 0.584 ≈ out-seq 0.567**(mean ΔzX는 out-seq 0.0270 > in-seq 0.0098로 *오히려 큼*) = 대조 상쇄 예측 그대로. → "threshold 30%"는 대조손실에 안 맞는 임의 휴리스틱. **self-reconstruction을 이유로 한 FORBIDDEN은 철회한다.**
> - **★단 코드검증 조건부 (E-1 규율)**: 위 (b)는 plan SPEC·v0.4.16 입증에 근거하지만, *이번 `L_mem` 경로*에서 SASRec `requires_grad=False`가 실제로 걸려 있고 `Δz`의 gradient가 prefix로만 흐르고 `seq` 경로에 안 닿음을 코드로 재확인하기 전엔 "불명(코드 미확인)"으로 둔다. 확인되면 정정 최종 확정.
> - **★진짜 게이트 = code-audit blocker (`X_in_seq` 아님)**: full training을 막아야 할 실제 사유는 — (i) **align_pairs manifest mismatch**(실제 52,236행 vs manifest 45행 = 재현성 붕괴), (ii) **source_text vs intent_description** — 로더가 spec의 `embedding.source_text` 대신 `intent_description`만 쓰면 `h_memory`가 *다른 벡터*가 되어 **모든 Δz 측정이 틀린 메모리 위에서 산출** → 변수결정 오염. ★단 `f3_bank.jsonl` 실제 필드 + 로더를 코드로 확인하기 전엔 이게 *진짜 discrepancy인지 non-issue인지 불명*(v0.4.3 compact-first로 persona/summary가 ablation 강등됐다면 `source_text≈intent_description`일 수 있음). discrepancy로 판명되면 현 ckpt가 `intent_description`으로 학습됐으므로 *재인코딩/재학습 필요여부*까지 결정. (iii) **runtime guard 부재**(forbidden β를 코드가 안 막음) · stale P2 generator · `--use_contrastive` 잔존(E-2 유령). **게이트는 이것들이지 `X_in_seq` 87.7%가 아니다.**
> - **★측정 자기정정 (Δz_Wtrain "미측정" 발언 철회)**: 직전 설계 분석에서 "Δz_Wtrain 미측정"이라 했으나 — v0.4.22 STEP1(이 changelog 아래 v0.4.22 블록)에서 *이미 측정됨*: `W_train` contrast가 A+B에서 양수(2a +0.018 / 2b +0.029), C+D에서 음수. → **`Wtrain_all` 폐기는 옳고, `Wtrain_aligned`(A+B)는 측정-양수.** `L_mem_X`도 측정-양수(`pos_frac` 0.58). **둘 다 유효 — 어느 쪽도 사전 dominant 아님.**
> - **★main 변수 = 학습 후 경험적 β 비교로 이연 (사전 anoint 금지)**: `L_mem_X`(= circular/retrieval 타깃, X-target SE14와 정합. target-derived eval에서 *상한*. 단 honest 쿼리에선 *과거* 아이템을 올려 직접 추천기여는 약함) vs `L_mem_Wtrain_aligned`(= honest recommendation 타깃, 단 A+B = `W_train`이 `m+` 클러스터에 든 *in-cluster transition*으로 ~49%·K≥2 제약). 둘 다 별도 β(`β_x`/`β_w`)로 ablation, 한 번에 하나. **논문 헤드라인 후보는 honest 쪽(`Wtrain_aligned`)** — `X`는 circularity 때문에 mechanism/상한 성격(self-reconstruction 아님). → v0.4.22의 "main=`L_mem_X`"와 외부권고의 "main=`Wtrain`(self-recon 근거)" *둘 다* 사전 anoint라 보류. main은 blocker 해결 + 올바른 `h_memory`에서 Δz 재확인 + β 학습 비교 *후* 확정.
> - **판정기준 불변 (top-K 아님)**: vanilla NDCG@10 0.0247은 sparse-Amazon full-ranking 문헌밴드(0.017~0.037; Beauty 0.032·Sports 0.017·Toys 0.037) *안* = backbone 정상 → floor(70.9% rank200+)는 *진짜 데이터 성질*. 성공기준 = honest-query 경로의 *방향 획득* + top-K는 floor 진단치와 *함께* 부차보고, 절대효과크기 동반. (PPR도 sparse/cold-start에서 빛남 = 우리 floor reality와 정합 — 후술 유사사례.)
> - **다음 순서 (학습 0)**: (1) v0.4.22 self-recon FORBIDDEN 정정 명문화[이 블록] → (2) frozen/gradient + source_text 실태 *코드검증* → (3) blocker fix(manifest·source_text·guard) → (4) 올바른 `h_memory`에서 Δz_X/Δz_Wtrain 재확인(샘플수·honest/circular 분해) → (5) `β_x`/`β_w` 각각 ablation 학습. **재학습·β sweep·full eval·artifact 본문수정은 (3)까지 금지.**
