...
**★L_mem 타깃 정합성 측정 및 v0.4.22 설계 정정 — main=`L_mem_X`, ablation=`L_mem_Wtrain_aligned`**:
...
- **남은 gate**: STEP2 smoke는 4모드(base/x-only/wtrain-only/combined) 통과 보고가 있었으나, full training 전 **`X_in_seq` self-reconstruction 감사**와 β=0 backward-compatibility 감사가 필요하다. 특히 `L_mem_X`에서 `X`가 이미 input `seq`에 포함되는 비율이 높으면, main 손실은 `seq_before_X` 구성 또는 `X==W_train` 제한 등으로 재정의해야 한다. 이 gate 통과 전 재학습 금지.
...
- `train_hybrid.py` (Stage 2): F6a 체크포인트(`checkpoints/{exp_name}/sasrec_pretrain.pt`)에서 SASRec을 로드(**기본값: frozen**, 1e-2x LR은 ablation) + text_encoder(저-LR, F6 체크포인트에서 초기화) + projector(**강-LR — v0.4.14 LLaVA 비대칭 튜닝: 인코더 저-LR / projector 강-LR로 modality gap 해소 주력, catastrophic forgetting 방지**) 동시 학습. 기본 loss는 `L = L_retrieval + α·L_align`이며, v0.4.22 이후 **선택적 score-space memory contrast**를 별도 β로 추가한다: `L = L_retrieval + α·L_align + β_x·L_mem_X + β_w·L_mem_Wtrain_aligned`. 기본값은 `β_x=β_w=0`으로 기존 SPEC과 동일해야 한다. `L_mem_X`는 `q`의 source item `X`와 provenance `m+`를 이용한 main 후보, `L_mem_Wtrain_aligned`는 A+B(`W_train`이 `m+`와 정합)에서만 쓰는 ablation이다. M2 메모리 뱅크에서 매 스텝 유저의 top-1 메모리를 router로 선택해 inference path를 유지하되, **L_mem의 positive는 router top-1이 아니라 provenance `positive_memory_id`**를 사용한다. `h_query`, `h_memory` 둘 다 **on-the-fly로 현재 text_encoder를 통해 재인코딩**(§7 결정 #11) → `projector(h_query, h_memory)` → prefix(P=1) → SASRec. **(v0.4.22 gate)** full training 전 `X_in_seq` self-reconstruction 감사와 β=0 backward-compatibility 감사를 통과해야 한다.
