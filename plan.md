# QAIM-Rec: Intent-Memory-Steered SASRec — 설계 문서 (v0.4.7, FINAL)
...
{purpose, is_discriminative,         
 preference_attrs}  ──┐               
            ...
            (distance threshold τ_personal, k_min=1 ≤ K_personal ≤ k_max=5)   # (v0.4.7) k_min=1 — 억지 K≥2 금지
                      │
                      ▼
...
if K_personal >= k_min(=1):              # (v0.4.7) k_min=1 — K_personal=1(single-intent)도 그대로 보고, prototype 패딩 금지
    k_prototype = 0  (보충 불필요)
else:  # K_personal == 0 인 경우에만 fallback
    anchor = 전체 구매이력(purchase-history) 임베딩의 centroid
    _prototypes.json에서 anchor와 cosine sim 상위 1개 prototype 추가(is_prototype=true 명시)
    k_prototype = 추가된 prototype 개수

memories = personal ++ 추가된 prototype  → IntentMemoryBank.memories (라우팅 후보 전체)
# (v0.4.7) K_personal=1을 prototype으로 2개 채우던 옛 동작 폐기 — single-intent는 정직히 1개로 보고(§7 #1, eval 층화 K=0/1/≥2 정합).
```
...
- `prompts/p1_intent.py`, `p2_pseudo_query.py`, `p3_align_judge.py`: 템플릿 + JSON 파싱/검증/재시도(파싱 실패시 최대 2회 재시도, 최종 실패는 별도 로그). **(v0.3)** `p1_intent.py`는 `disposition_note`(§5) 필드를 추가로 출력 — M2 `synth.py`의 persona synthesis 입력으로 사용. **(v0.4.6)** P1 출력 레코드는 `item_id`/`timestamp`/`rating`을 1급 필드로 함께 기록하고, 병렬 워커 shard는 즉시 단일 정본 `p1_extractions.jsonl`(+manifest md5)로 머지 — 다운스트림은 이 정본만 읽음. `p2_pseudo_query.py`는 **train/eval 2-변형**(`p2_pseudo_query_train.txt`/`p2_pseudo_query_eval.txt`)을 로드하며, 입력에 item 메타를 *맥락*으로 넣되 title/brand/저자가 출력 쿼리에 나오면 reject+retry(≤2) 후 사후 디텍터로 재검(§6.5).
...
- `bank.py`: `IntentMemoryBank.load(user_id)`, `.save()`, `.route(query_emb, top_k=1) -> List[IntentMemoryUnit]` (cosine sim, personal+prototype 후보 전체 대상). **`assemble(user_id, k_min=1)`** (v0.4.7): K_personal==0일 때만 `_prototypes.json`에서 전체 구매이력 임베딩 centroid와 가장 가까운 1개 prototype을 추가해 `k_prototype` 채움. K_personal≥1은 personal 그대로(single-intent도 패딩 금지).
